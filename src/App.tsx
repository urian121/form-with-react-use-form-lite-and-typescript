...
interface MyFormData {
  ...
  time_now: string,
  pais: string,
  teGustaReact: boolean,
  aceptaTerminos: string,
  tareas: string,
  password: string,
  telefono: string,
  url_web: string,
  datetime_now: string | null;
  month_now: string,
  week_now: string,
  my_color: string,
  my_search: string,
...
function App() {
  const initialFormValues: MyFormData = {
  ...
  const {
    ...
    resetForm,
    getEmptyFields
  } = useFormLite<MyFormData>(initialFormValues, handleFormSubmit);

  return (
    <>
      <h1 style={{ textAlign: 'center', margin: '40px 0' }}>
        Formulario con <code>react-use-form-lite</code> y <code style={{ color: '#61DAFB' }}>TypeScript</code>
      </h1>
